...
saved_dict(
	...
	"""
	from seed_.saved_dict import saved_dict
...
def mk_cache(n):
		...
		######
		if 1:
			_this_ = "symmetric_poly2basic"
			addr_infile = f"sv[{n_repr}]addr.{_this_}.py.txt"
			items_infile = f"sv[{n_repr}]items.{_this_}.py.txt"
			items_picklefile = f"sv[{n_repr}]pickle.{_this_}.py.data"
			kwargs = dict(n=n_repr, gen=f"{_this_}.py")
