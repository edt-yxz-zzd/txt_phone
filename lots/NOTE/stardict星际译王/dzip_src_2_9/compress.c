...
#endif
		printf(": negative compression; storing at 0%%\n");
		dzFile_Seek(de->ptr);
...
void dzCompressFile (char *name, uInt filesize, uInt filedate)
{
	...
	if (filetype == TYPE_PAK)
	{
		...
		if ((pakheader.offset + pakheader.size != filesize) 
			|| (pakheader.size & 0x3f))
		{
			fprintf(stderr, "warning: %s has problems\n", name);
			goto notapak;
		}
		...
		for (i = 1; i <= pakheader.size; i++)
		{
			...
			if (filetype == TYPE_PAK)
			{
				fprintf(stderr, "warning: %s inside %s will be "
		...
		#endif
			printf(" compressing %s", entry.name);
